...
public class JmsConnectionErrorHandlerTest {
  ...
  public void testConnectionErrorHandler(TestInfo info) throws Exception {
    MockChannel channel = createChannel(activeMqBroker, activeMqBroker.getJmsConnection(),
  ...
  public void testConnectionErrorHandler_WithRuntimeException(TestInfo info) throws Exception {
    MockChannel channel = createChannel(activeMqBroker,
        new JmsConnectionCloseWithRuntimeException(activeMqBroker.getJmsConnection()),
        info.getDisplayName(), new JmsConnectionErrorHandler() {
          @Override
  ...
  public void testConnectionErrorHandler_WithInitialiseException(TestInfo info) throws Exception {
    MockChannel channel = new MockChannelFail(createChannel(activeMqBroker,
        activeMqBroker.getJmsConnection(),
        info.getDisplayName(), new JmsConnectionErrorHandler()),
  ...
  public void testRestartSharedConnection(TestInfo info) throws Exception {
    ...
    JmsConnection connection = activeMqBroker.getJmsConnection();
  ...
  private MockChannel createChannel(EmbeddedArtemis mq, JmsConnection con, String destinationName, JmsConnectionErrorHandler h)
      throws Exception {
    return createChannel(mq.getName() + "_channel", mq, con, destinationName, h);
  }

  private MockChannel createChannel(EmbeddedArtemis mq, SharedConnection con, String destinationName) throws Exception {
  ...
  private MockChannel createChannel(String channelName, EmbeddedArtemis mq, JmsConnection con, String destinationName)
      throws Exception {
  ...
  private Workflow createWorkflow(EmbeddedArtemis mq, String destination) throws Exception {
  ...
  private StandaloneProducer createProducer(EmbeddedArtemis mq, String dest) {
    JmsConnection conn = mq.getJmsConnection();
